...
from app.models import ExportRecord, ExtractionResult, ReviewEdit, ExtractionJob, TemplateVersion
...
def export_result(db: Session, result: ExtractionResult, export_format: str) -> ExportRecord:
    ...
    path = Path(settings.exports_dir) / f"result-{result.id}-{timestamp}.{ 'xlsx' if export_format == 'excel' else export_format }"
    ...
    if export_format == "json":
        ...
        with path.open("w", newline="", encoding="utf-8") as handle:
            ...
            for field in summary.extracted_fields:
                writer.writerow([field.field_name, field.label, "extracted", json.dumps(field.normalized_value), field.validation_status, field.requires_review])
            for field in summary.calculated_fields:
                writer.writerow([field.field_name, field.label, "calculated", json.dumps(field.calculated_value), field.validation_status, field.requires_review])
        ...
        for field in summary.extracted_fields:
            sheet.append([field.field_name, field.label, "extracted", json.dumps(field.normalized_value), field.validation_status, field.requires_review])
        for field in summary.calculated_fields:
            sheet.append([field.field_name, field.label, "calculated", json.dumps(field.calculated_value), field.validation_status, field.requires_review])
        workbook.save(path)
    else:
