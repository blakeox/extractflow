...
test("keeps a trusted result in place when export generation fails", async ({ page }) => {
  ...
  await expect(page.getByText("Export generation failed at the backend.")).toBeVisible();
  await expect(page.getByRole("heading", { name: "Extraction complete" })).toBeVisible();
